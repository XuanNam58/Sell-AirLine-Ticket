...
//            " WHERE t.flight.flightID = :flightID)")
    ...
    @Query("FROM Seat s " +
            "INNER JOIN Ticket t ON s.seatNum = t.seatNum AND t.flight.flightID = :flightID " +
            "WHERE t.flight.flightID = :flightID")
    List<Seat> getSeatsInTicket(@Param("flightID") Integer flightID);

    @Query("FROM Seat s " +
            "INNER JOIN Flight f ON s.plane.planeID = f.plane.planeID " +
...
//            "           AND seatNum = (SELECT seatNum FROM Seat WHERE seatID = :seatID)) AS t ON s.userID = t.user.userID ")
