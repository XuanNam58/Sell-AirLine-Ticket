    ...
    public void run(String... args) throws Exception {
        if (userRepository.count() == 0){
            userRepository.saveAll(List.of(
                    User.builder().userID("0001").name("Nguyen Van An")
                            .phoneNum("0123456789").email("vanan@gmail.com")
                            .citizenID("0123123123").creditNum("0111222333")
                            .status(true).build(),
                    User.builder().userID("1001").name("Tran Ngoc Ba")
                            .phoneNum("0123321123").email("ngocba@gmail.com")
                            .citizenID("1234565432").creditNum("0567567567")
                            .status(true).build()
            ));
        }
        ...
        if (accountRepository.count()==0){
            ...
            accountRepository.saveAll(List.of(
                    Account.builder().username("customer01")
                            ...
                            .user(user.get()).status(true).build()
            ));
            }
